...
const SchemeDetail: React.FC = () => {
  ...
  // ✅ FIX 1: redirect logic useEffect me
  useEffect(() => {
    if (!isAuthenticated && id) {
      navigate('/login', { state: { returnUrl: `/scheme/${id}` } });
    }
  }, [isAuthenticated, id, navigate]);

  // ✅ FIX 2: recently viewed bhi useEffect me safe
  useEffect(() => {
    if (isAuthenticated && id) {
      ...
      <main className="flex-1 py-8">
        <div className="container max-w-4xl">
          ...
          <Button
            ...
            onClick={() => {
              if (scheme?.state) {
                navigate(`/state-schemes/${encodeURIComponent(scheme.state)}`);
              } else {
                navigate('/state-schemes');
              }
            }}
            className="mb-6"
          >
          ...
          <div className="bg-card rounded-2xl border overflow-hidden">
            <div className="gradient-hero p-8">
              <span
                className={`px-3 py-1 text-sm rounded-full ${scheme.type === 'Central'
                    ? 'bg-white/20'
                    : 'bg-accent/80'
                  } text-white`}
              >
