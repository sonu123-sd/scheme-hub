...
import { 
  User, FileText, Bookmark, Clock, Settings, Upload, 
  Camera, Edit2, Save, X, Trash2, Eye, CheckCircle2, Users, Calendar
} from 'lucide-react';
...
const Dashboard = () => {
  ...
  const handleDocumentUpload = (docType: string) => (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        const documents = { ...user?.documents, [docType]: reader.result as string };
  ...
  const handleDeleteDocument = (docType: string) => {
    ...
    delete documents[docType as keyof typeof documents];
    updateUser({ documents });
  ...
  const eligibleSchemes = useMemo(() => {
    if (!user?.age || !user?.gender) return [];
    ...
    return schemes.filter(scheme => {
      ...
      const genderEligible = eligibility.gender === 'All' || 
              ...
              <CardHeader>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  <Badge className={userAgeCategory?.color}>
                    ...
                    {user?.age} years - {userAgeCategory?.label}
                  </Badge>
              ...
              <CardContent>
                <div className="flex flex-col md:flex-row gap-8">
                  ...
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <Label>Age</Label>
                      <p className="mt-1 p-2 bg-muted rounded">{user?.age} years</p>
                    </div>
                    <div>
                      <Label>Gender</Label>
                      {isEditing ? (
